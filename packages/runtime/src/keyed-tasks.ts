...
export interface TaskHandle<A, E> {
  readonly outcome: Promise<TaskOutcome<A, E>>;
}
  ...
  return {
    submit(key: Key, input: Input | Snapshot<Input>): TaskHandle<A, E> {
      if (disposed || owner.disposed)
        return { outcome: Promise.resolve(Object.freeze({ _tag: 'Cancelled' as const })) };
      const captured = protectSnapshot(input) as Snapshot<Input>;
      const entry = get(key);
      entry.version++;
      let settle!: (outcome: TaskOutcome<A, E>) => void;
      const outcome = new Promise<TaskOutcome<A, E>>((resolve) => {
        settle = (result) => resolve(protectSnapshot(result));
      });
      outcome
        .then(async () => {
          ...
          do {
            ...
            await entry.source.awaitStopped();
        ...
        .catch((error: unknown) => reportSafely(reportError, error));
