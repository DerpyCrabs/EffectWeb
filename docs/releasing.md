...
Runtime, compiler, Lucide, and the five native packages use one version. Native packages contain only a binary, package metadata, README, and license. Consumers install the platform package through the compiler's optional dependencies; they do not need Cargo or install scripts.
...
It runs CI, builds and tests the native compiler on all five platforms, and creates an `npm-packages` artifact containing eight tarballs. Packaging rejects missing binaries and mismatched versions. No npm credentials are needed for this path.
...
3. Run `node scripts/publish-release.mjs`. It checks that all eight tarballs exist, publishes native packages first, then `effectweb`, `@effectweb/compiler`, and `@effectweb/lucide`. Existing versions are skipped so interrupted publication can resume. Check the registry if a name has been claimed since setup.
4. With npm 11.19+, configure each package through the CLI:
   ...
   Alternatively, on npm configure a GitHub Actions trusted publisher for **each of the eight packages** with owner `DerpyCrabs`, repository `EffectWeb`, workflow filename `release.yml`, and environment `npm`. Enable direct `npm publish` in the trusted publisher's allowed actions.

The packages are `effectweb`, `@effectweb/compiler`, `@effectweb/lucide`, and `@effectweb/compiler-` followed by `linux-x64-gnu`, `linux-arm64-gnu`, `darwin-x64`, `darwin-arm64`, or `win32-x64-msvc`.
