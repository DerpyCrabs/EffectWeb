...
- Optional async `validate(value)` receives deeply readonly parsed data and returns an Effect of an error string or `undefined`. Parsing failures skip it. Typed failures and defects become a retryable display error; customize this with `onFailure(cause)`. A new edit, reset or validation cancels the old command. Revision checks also ignore stale and duplicate settlement messages.
